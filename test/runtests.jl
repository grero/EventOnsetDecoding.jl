...
@testset begin "Basic"
    ...
    #FIXME: This does not necessarily work so well since we'll be substracting the mean
    X[8,:,:] .+= μ 
    X[9,:,:] .+= 0.75*μ
    X[10,:,:] .+= 0.5*μ
    ...
    dargs = EventOnsetDecoding.DecoderArgs(["animalZ/20140904/session01"],[1],windows=[5.0], latencies=[0.0], 
                                            difference_decoder=true,use_area="ALL",rtime_min=0.0, rtime_max=1.0,
